...
fn check_game_status() {
    for row in 0..10 {
        for col in 0..10 {
            ...
            assert_eq!(rc.borrow_mut().check_game_status(), GsGameContinue);              // O _ X O _
                                                                                          // _ X O _ X
            board.borrow_mut().put_chess_at(Coord{row: row + 4, col: col + 0}, CtWhite);  // X O _ X O
            assert_eq!(rc.borrow_mut().check_game_status(), GsGameOver(CtWhite));         // O _ X O _
            assert_eq!(rc.borrow().game_status(), GsGameOver(CtWhite));
            ...
            assert_eq!(rc.borrow_mut().check_game_status(), GsGameOver(CtBlack));         // O _ X O X
            assert_eq!(rc.borrow().game_status(), GsGameOver(CtBlack));
            ...
            assert_eq!(rc.borrow_mut().check_game_status(), GsGameOver(CtBlack));         // X O X X O
            assert_eq!(rc.borrow().game_status(), GsGameOver(CtBlack));                   // O _ X O X
            ...
            assert_eq!(rc.borrow_mut().check_game_status(), GsGameOver(CtWhite));         // O _ X O X
            assert_eq!(rc.borrow().game_status(), GsGameOver(CtWhite));
        }
    }
}