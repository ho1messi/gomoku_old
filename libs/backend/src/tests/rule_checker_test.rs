...
fn check_game_status() {
    for row in 0..10 {
        for col in 0..10 {
            let board_rc = Rc::new(RefCell::new(Board::new()));
            let rule_checker_rc = Rc::new(RefCell::new(
                RuleChecker::create_with_detail(Rc::downgrade(&board_rc))));
            let mut board = board_rc.borrow_mut();
            let mut rule_checker = rule_checker_rc.borrow_mut();

            //board.add_observers(Rc::downgrade(&rule_checker_rc));
...
fn update_evaluation_by_event() {
    ...
    board.put_chess_at(Coord{row: 8, col: 8}, CtBlack);
